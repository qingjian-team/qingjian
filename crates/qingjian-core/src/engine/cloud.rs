//! 云联想：组句中发请求、收结果校验、Tab 接受整句；翻译选中文字也走这里。
...
impl Engine {
    ...
    /// （[`prediction::translation_target`]）。云联想关着、文字为空时不发，返回 `None`；
    ...
    pub fn request_translation(&mut self, text: &str) -> Option<u64> {
        ...
        let request = PredictionRequest {
            ...
            want_sentence: true,
            text: text.to_owned(),
            target_language: prediction::translation_target(text, self.translator.language())
                .code()
                .to_owned(),
        };
        ...
        while let Some(mut prediction) = self.predictor.poll() {
            if prediction.sequence == self.prediction_sequence {
                ...
                if self.last_prediction_kind == PredictionKind::Question {
                    ...
                    prediction
                        .words
                        .retain(|word| !prediction::restates_question(&word.text, guess));
        ...
        match sentence::segment_text(text, &*self.language_model) {
            Some(clauses) => {
                ...
                for (index, words) in clauses.iter().enumerate() {
                    ...
                    for word in words {
                        self.record_word(word, &[], false, buffer_left || index + 1 < count);
                    }
                }
