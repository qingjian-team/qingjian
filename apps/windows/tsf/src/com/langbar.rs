//! 中 / 英 输入模式指示器：Win11 托盘品牌图标左边的模式图标。按微软 IME 要求，通过 `GUID_LBI_INPUTMODE`
//! 的语言栏按钮把「中 / 英」图标交给系统输入指示器（转换模式 compartment 不走这条通道，所以之前不显示）。
//! 单击 Shift 或点这个按钮都切模式；切完 [`ModeState::set_english`] 通知系统重取图标。

use core::cell::{Cell, RefCell};
use std::rc::Rc;

use windows::Win32::Foundation::{COLORREF, E_NOINTERFACE, POINT, RECT};
use windows::Win32::Graphics::Gdi::{
    BI_RGB, BITMAPINFO, BITMAPINFOHEADER, CLEARTYPE_QUALITY, CLIP_DEFAULT_PRECIS, CreateBitmap,
    CreateCompatibleDC, CreateDIBSection, CreateFontW, DEFAULT_CHARSET, DIB_RGB_COLORS, DT_CENTER,
    DT_SINGLELINE, DT_VCENTER, DeleteDC, DeleteObject, DrawTextW, FF_DONTCARE, FW_NORMAL,
    OUT_TT_PRECIS, SelectObject, SetBkMode, SetTextColor, TRANSPARENT, VARIABLE_PITCH,
};
use windows::Win32::UI::TextServices::{
    GUID_LBI_INPUTMODE, ITfLangBarItem_Impl, ITfLangBarItemButton, ITfLangBarItemButton_Impl,
    ITfLangBarItemSink, ITfMenu, ITfSource, ITfSource_Impl, TF_LANGBARITEMINFO, TF_LBI_ICON,
    TF_LBI_STATUS, TF_LBI_STYLE_BTN_BUTTON, TfLBIClick,
};
use windows::Win32::UI::WindowsAndMessaging::{CreateIconIndirect, HICON, ICONINFO};
use windows::core::{BOOL, BSTR, GUID, IUnknown, Interface, Ref, Result, implement, w};

use super::CLSID_QINGJIAN;

/// 当前中英模式 + 语言栏更新回调，DLL 与语言栏按钮共享（STA 单线程，不并发）。
pub(crate) struct ModeState {
    /// `true` 是英文模式。
    english: Cell<bool>,

    /// 系统登记进来的语言栏更新回调；模式变了通知它重取图标。
    sink: RefCell<Option<ITfLangBarItemSink>>,
}

impl ModeState {
    pub(crate) fn new() -> Rc<Self> {
        Rc::new(Self {
            english: Cell::new(false),
            sink: RefCell::new(None),
        })
    }

    /// 切模式后同步指示器：改状态并通知系统重取图标 / 文字。
    pub(crate) fn set_english(&self, english: bool) {
        self.english.set(english);
        if let Some(sink) = self.sink.borrow().as_ref() {
            // SAFETY: sink 是系统经 AdviseSink 登记的回调。
            let _ = unsafe { sink.OnUpdate(TF_LBI_ICON | TF_LBI_STATUS) };
        }
    }
}

/// `GUID_LBI_INPUTMODE` 语言栏按钮，图标随 [`ModeState`] 显示中 / 英，点它切模式。
#[implement(ITfLangBarItemButton, ITfSource)]
pub(crate) struct ModeButton {
    state: Rc<ModeState>,
}

impl ModeButton {
    /// 造一个中 / 英按钮，返回它的语言栏接口（登记给系统语言栏管理器）。
    pub(crate) fn create(state: Rc<ModeState>) -> ITfLangBarItemButton {
        Self { state }.into()
    }
}

impl ITfLangBarItem_Impl for ModeButton_Impl {
    fn GetInfo(&self, pinfo: *mut TF_LANGBARITEMINFO) -> Result<()> {
        // SAFETY: 系统传入的可写指针。
        let info = unsafe { &mut *pinfo };
        info.clsidService = CLSID_QINGJIAN;
        info.guidItem = GUID_LBI_INPUTMODE;
        info.dwStyle = TF_LBI_STYLE_BTN_BUTTON;
        info.ulSort = 0;
        let desc: Vec<u16> = "青简中英模式".encode_utf16().collect();
        let n = desc.len().min(info.szDescription.len());
        info.szDescription[..n].copy_from_slice(&desc[..n]);
        Ok(())
    }

    fn GetStatus(&self) -> Result<u32> {
        Ok(0)
    }

    fn Show(&self, _fshow: BOOL) -> Result<()> {
        Ok(())
    }

    fn GetTooltipString(&self) -> Result<BSTR> {
        Ok(BSTR::from("中 / 英（单击 Shift 切换）"))
    }
}

impl ITfLangBarItemButton_Impl for ModeButton_Impl {
    fn OnClick(&self, _click: TfLBIClick, _pt: &POINT, _prcarea: *const RECT) -> Result<()> {
        super::service::on_shift_tap();
        Ok(())
    }

    fn InitMenu(&self, _pmenu: Ref<ITfMenu>) -> Result<()> {
        Ok(())
    }

    fn OnMenuSelect(&self, _wid: u32) -> Result<()> {
        Ok(())
    }

    fn GetIcon(&self) -> Result<HICON> {
        make_mode_icon(if self.state.english.get() {
            '英'
        } else {
            '中'
        })
    }

    fn GetText(&self) -> Result<BSTR> {
        Ok(BSTR::from(if self.state.english.get() {
            "英"
        } else {
            "中"
        }))
    }
}

impl ITfSource_Impl for ModeButton_Impl {
    fn AdviseSink(&self, riid: *const GUID, punk: Ref<IUnknown>) -> Result<u32> {
        // SAFETY: riid 是系统传入的有效 GUID 指针。
        if unsafe { *riid } != ITfLangBarItemSink::IID {
            return Err(E_NOINTERFACE.into());
        }
        let sink: ITfLangBarItemSink = punk.ok()?.cast()?;
        *self.state.sink.borrow_mut() = Some(sink);
        Ok(1) // 只支持一个回调，cookie 固定。
    }

    fn UnadviseSink(&self, _dwcookie: u32) -> Result<()> {
        *self.state.sink.borrow_mut() = None;
        Ok(())
    }
}

/// 画模式图标：透明底、白字的「中」/「英」（Win11 深色托盘可见）。画得比托盘实际尺寸大，系统缩小显示更清晰。
/// 系统取走 HICON 后负责销毁。
fn make_mode_icon(ch: char) -> Result<HICON> {
    // 画布放大到 24（托盘约 16–20，缩小更锐）；字高用负值（精确字符高度，不含行距），几乎填满画布。
    const SIZE: i32 = 24;
    let header = BITMAPINFOHEADER {
        biSize: core::mem::size_of::<BITMAPINFOHEADER>() as u32,
        biWidth: SIZE,
        biHeight: -SIZE, // 负高 = top-down
        biPlanes: 1,
        biBitCount: 32,
        biCompression: BI_RGB.0,
        ..Default::default()
    };
    let bmi = BITMAPINFO {
        bmiHeader: header,
        ..Default::default()
    };
    let mut bits: *mut core::ffi::c_void = core::ptr::null_mut();
    // SAFETY: 一串标准 GDI 调用，句柄用完各自释放；bits 由 CreateDIBSection 指向 SIZE*SIZE*4 字节。
    unsafe {
        let color = CreateDIBSection(None, &bmi, DIB_RGB_COLORS, &mut bits, None, 0)?;
        let memdc = CreateCompatibleDC(None);
        let old_bmp = SelectObject(memdc, color.into());
        let font = CreateFontW(
            -(SIZE - 2), // 负字高 = 精确字符高度，字几乎填满画布
            0,
            0,
            0,
            FW_NORMAL.0 as i32,
            0,
            0,
            0,
            DEFAULT_CHARSET,
            OUT_TT_PRECIS,
            CLIP_DEFAULT_PRECIS,
            CLEARTYPE_QUALITY,
            (VARIABLE_PITCH.0 | FF_DONTCARE.0) as u32,
            w!("Microsoft YaHei UI"),
        );
        let old_font = SelectObject(memdc, font.into());
        let _ = SetBkMode(memdc, TRANSPARENT);
        SetTextColor(memdc, COLORREF(0x00FF_FFFF)); // 白字
        let mut rect = RECT {
            left: 0,
            top: 0,
            right: SIZE,
            bottom: SIZE,
        };
        let mut text: Vec<u16> = ch.to_string().encode_utf16().collect();
        DrawTextW(
            memdc,
            &mut text,
            &mut rect,
            DT_CENTER | DT_VCENTER | DT_SINGLELINE,
        );
        // GDI 画字不写 alpha：把画上字的像素（RGB 非 0）补成不透明，其余保持透明。
        let pixels = bits as *mut u32;
        for i in 0..(SIZE * SIZE) as isize {
            let p = *pixels.offset(i);
            if p & 0x00FF_FFFF != 0 {
                *pixels.offset(i) = p | 0xFF00_0000;
            }
        }
        SelectObject(memdc, old_font);
        let _ = DeleteObject(font.into());
        SelectObject(memdc, old_bmp);
        let _ = DeleteDC(memdc);
        // 掩码全 0（靠 32bpp 的 alpha 决定透明）。
        let mask = CreateBitmap(SIZE, SIZE, 1, 1, None);
        let info = ICONINFO {
            fIcon: true.into(),
            xHotspot: 0,
            yHotspot: 0,
            hbmMask: mask,
            hbmColor: color,
        };
        let icon = CreateIconIndirect(&info);
        let _ = DeleteObject(mask.into());
        let _ = DeleteObject(color.into());
        icon
    }
}
