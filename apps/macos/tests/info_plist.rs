...
//!
//! 缺 `tsInputModeListKey`（输入模式声明）时：标准文本视图（备忘录等）按 script / 模式过滤输入源，
...
/// 输入模式声明：每个模式的 ID、语言、repertoire、script 与菜单图标都要与顶层声明对得上。
/// 声明里具体几个模式不限定（将来加繁体模式也过），但结构缺一项就是当年的坑。
#[test]
fn declares_input_modes_for_standard_text_views() {
    ...
    let modes = plist
        ...
        .expect(
            "缺 tsInputModeListKey：没有输入模式声明，标准文本视图（备忘录等）切换不过去，\
    ...
    for (mode_id, value) in modes {
        ...
        assert_eq!(
            string(mode, "tsInputModeMenuIconFileKey"),
            icon,
            "模式菜单图标与输入法菜单图标用同一个文件（打包脚本只生成这一个 tiff）"
        );
    }
}
